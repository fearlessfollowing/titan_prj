...
Trans::Trans(const sp<ARMessage>& notify): mNotify(notify)
{
	init();
}
...
char* Trans::postSyncMessage(const char* data)
{
	...
	msg->set<int>("what", TRANS_NOTIFY_MSG);
	msg->set<int>("data", recvBuf);
	...
	while (true) {
        ...
        if (len != FIFO_HEAD_LEN) {	/* 头部读取错误 */
			...
			if (msg_what == CMD_EXIT) { /* 如果是退出消息 */
				...
				if (len != content_len) {	/* 读取的数据长度不一致 */
                    Log.e(TAG, "read fifo content mismatch(%d %d)", len, content_len);
					break;
                } else {
					memset(recvBuf, 0, sizeof(recvBuf));
					memcpy(recvBuf, &buf[FIFO_HEAD_LEN], content_len);
