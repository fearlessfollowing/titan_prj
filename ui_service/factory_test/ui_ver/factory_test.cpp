...
void FactoryTest::sendExit()
{
    ...
    if(!bExitMsg)
    {
        ...
        if (th_msg_.joinable())
        {
//            Log.d(TAG," send oled exit");
            obtainMessage(0xff)->post();
//            Log.d(TAG," send oled exit2");
            th_msg_.join();
        }
        else
        {
...
void FactoryTest::handleMessage(const sp<ARMessage> &msg)
{
	...
	case FACTORY_TEST_LED:
		...
		mOLEDModule->ssd1306_disp_16_str((const u8*)"LED Test Menu", 2, 12);
		ret = mOLEDLight->factory_test();
		
		const u8* pret = (ret != 0) ? (const u8 *)"LED Test Failed" : (const u8 *)"Led Test OK";
		mOLEDModule->ssd1306_disp_16_str(pret, 8, 24);
		break;
