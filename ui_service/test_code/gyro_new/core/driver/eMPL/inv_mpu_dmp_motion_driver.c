...
{
//    printf("sizeof(dmp_memory) %lu\n",sizeof(dmp_memory)/sizeof(dmp_memory[0]));
...
#ifdef EMPL_NO_64BIT
    gyro_bias_body[0] = (int)(((float)gyro_bias_body[0] * GYRO_SF) / 1073741824.f);
    gyro_bias_body[1] = (int)(((float)gyro_bias_body[1] * GYRO_SF) / 1073741824.f);
    gyro_bias_body[2] = (int)(((float)gyro_bias_body[2] * GYRO_SF) / 1073741824.f);
#else
    gyro_bias_body[0] = (int)(((long long)gyro_bias_body[0] * GYRO_SF) >> 30);
    gyro_bias_body[1] = (int)(((long long)gyro_bias_body[1] * GYRO_SF) >> 30);
    gyro_bias_body[2] = (int)(((long long)gyro_bias_body[2] * GYRO_SF) >> 30);
#endif
...
#ifdef EMPL_NO_64BIT
    accel_bias_body[0] = (int)(((float)accel_bias_body[0] * accel_sf) / 1073741824.f);
    accel_bias_body[1] = (int)(((float)accel_bias_body[1] * accel_sf) / 1073741824.f);
    accel_bias_body[2] = (int)(((float)accel_bias_body[2] * accel_sf) / 1073741824.f);
#else
    accel_bias_body[0] = (int)(((long long)accel_bias_body[0] * accel_sf) >> 30);
    accel_bias_body[1] = (int)(((long long)accel_bias_body[1] * accel_sf) >> 30);
    accel_bias_body[2] = (int)(((long long)accel_bias_body[2] * accel_sf) >> 30);
#endif
...
{
    unsigned char tmp[4];
    int thresh_scaled = sf / 1000 * thresh;
    tmp[0] = (unsigned char)(((int)thresh_scaled >> 24) & 0xFF);
    tmp[1] = (unsigned char)(((int)thresh_scaled >> 16) & 0xFF);
    tmp[2] = (unsigned char)(((int)thresh_scaled >> 8) & 0xFF);
    tmp[3] = (unsigned char)((int)thresh_scaled & 0xFF);
...
int dmp_get_pedometer_step_count(unsigned int *count)
{
    ...
    count[0] = ((unsigned int)tmp[0] << 24) | ((unsigned int)tmp[1] << 16) |
        ((unsigned int)tmp[2] << 8) | tmp[3];
    return 0;
}
...
int dmp_get_pedometer_walk_time(unsigned int *time)
{
    ...
    time[0] = (((unsigned int)tmp[0] << 24) | ((unsigned int)tmp[1] << 16) |
        ((unsigned int)tmp[2] << 8) | tmp[3]) * 20;
    return 0;
}
...
int dmp_enable_feature(unsigned short mask)
{
 ...
 //   printf("dmp_enable_feature mask 0x%x\n",mask);
...
//    printf("dmp.feature_mask is 0x%x\n",dmp.feature_mask);
    mpu_reset_fifo();
...
//    printf("dmp.feature_mask is 0x%x dmp.packet_length %d\n",dmp.feature_mask,dmp.packet_length);
    /* Get a packet. */
...
//    printf("2dmp.feature_mask is 0x%x "
//                   "dmp.packet_length %d\n",
//           dmp.feature_mask,dmp.packet_length);
#ifdef FIFO_CORRUPTION_CHECK
//    unsigned char i = 0;
...
#ifdef FIFO_CORRUPTION_CHECK
        int quat_q14[4], quat_mag_sq;
#endif
        quat[0] = ((int)fifo_data[0] << 24) | ((int)fifo_data[1] << 16) |
            ((int)fifo_data[2] << 8) | fifo_data[3];
        quat[1] = ((int)fifo_data[4] << 24) | ((int)fifo_data[5] << 16) |
            ((int)fifo_data[6] << 8) | fifo_data[7];
        quat[2] = ((int)fifo_data[8] << 24) | ((int)fifo_data[9] << 16) |
            ((int)fifo_data[10] << 8) | fifo_data[11];
        quat[3] = ((int)fifo_data[12] << 24) | ((int)fifo_data[13] << 16) |
            ((int)fifo_data[14] << 8) | fifo_data[15];
        ii += 16;
#ifdef FIFO_CORRUPTION_CHECK
...
//        printf("quat_mag_sq is %d QUAT_MAG_SQ_MIN %ld "
//                       "QUAT_MAG_SQ_MAX %ld",
//               quat_mag_sq,QUAT_MAG_SQ_MIN, QUAT_MAG_SQ_MAX);
...
//    printf("DMP_FEATURE_SEND_ANY_GYRO 0x%x dmp.feature_mask  0x%x\n",
//           DMP_FEATURE_SEND_ANY_GYRO,dmp.feature_mask );
