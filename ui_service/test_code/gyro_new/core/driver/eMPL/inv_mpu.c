 ...
 * min(int a, int b)
 */
#if defined EMPL_TARGET_NV
#include "ins_i2c.h"
#define i2c_write   ins_i2c_write
#define i2c_read    ins_i2c_read
#define delay_ms    ins_delay_ms
#define get_ms      nv_get_clock_ms
#define log_i       printf
#define log_e       printf
//#define reg_int_cb  ins_reg_int_cb
#define min(a,b) ((a<b)?a:b)

static inline int reg_int_cb(struct int_param_s *int_param)
{
//    return ins_reg_int_cb(int_param->cb, int_param->pin, int_param->lp_exit,
 ...
 *  @return     0 if successful.
 */
int mpu_init(struct int_param_s *int_param,int bReset)
{
    unsigned char data[6];

    printf("mpu_init bReset %d\n",bReset);
    if(bReset == 1)
    {
        /* Reset device. */
        data[0] = BIT_RESET;
//        printf("1set pwr 0x%x\n",data[0]);
...
#ifdef AK89xx_SECONDARY
    printf("AK89xx_SECONDARY enable\n");
    setup_compass();
    if (mpu_set_compass_sample_rate(10))
        return -1;
#else
//    printf("AK89xx_SECONDARY disable\n");
...
int mpu_lp_accel_mode(unsigned short rate)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->lp_accel_odr, 1, tmp))
        return -1;
    tmp[0] = BIT_LPA_CYCLE;
    printf("6 set pwr 0x%x\n",tmp[0]);
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp))
        return -1;
#endif
    printf("mpu_lp_accel_mode rate %d\n",rate);
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_reg(short *data, unsigned long long *timestamp)
{
    unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_reg(short *data, unsigned long long *timestamp)
{
    unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(int *data, unsigned long long *timestamp)
{
    unsigned char tmp[2];
    short raw;
    ...
    data[0] = (int)((35 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
    return 0;
}
 ...
 *  @return     0 if successful.
 */
int mpu_read_6500_accel_bias(int *accel_bias) {
	unsigned char data[6];
	...
	if (i2c_read(st.hw->addr, 0x7D, 2, &data[4]))
		return -1;
	accel_bias[0] = ((int)data[0]<<8) | data[1];
	accel_bias[1] = ((int)data[2]<<8) | data[3];
	accel_bias[2] = ((int)data[4]<<8) | data[5];

    printf("mpu_read_6500_accel_bias "
 ...
 *  @return     0 if successful.
 */
int mpu_read_6050_accel_bias(int *accel_bias) {
	unsigned char data[6];
...
int mpu_read_6500_gyro_bias(int *gyro_bias) {
	unsigned char data[6];
	...
	if (i2c_read(st.hw->addr, 0x17, 2, &data[4]))
		return -1;
	gyro_bias[0] = ((int)data[0]<<8) | data[1];
	gyro_bias[1] = ((int)data[2]<<8) | data[3];
	gyro_bias[2] = ((int)data[4]<<8) | data[5];

    printf("mpu_read_6500_gyro_bias "
                   "(0x%x 0x%x 0x%x 0x%x 0x%x 0x%x)\n",
           data[0],data[1],data[2],data[3],data[4],data[5]);
	return 0;
}
 ...
 *  @return     0 if successful.
 */
int mpu_set_gyro_bias_reg(int *gyro_bias)
{
    unsigned char data[6] = {0, 0, 0, 0, 0, 0};
    int i=0;
    printf("set gyro (%d %d %d)\n",
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias_6050_reg(const int *accel_bias) {
    unsigned char data[6] = {0, 0, 0, 0, 0, 0};
    int accel_reg_bias[3] = {0, 0, 0};
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias_6500_reg(const int *accel_bias) {
    unsigned char data[6] = {0, 0, 0, 0, 0, 0};
    int accel_reg_bias[3] = {0, 0, 0};

    printf("accel_bias (%d %d %d)\n",
    ...
    if (i2c_write(st.hw->addr, 0x7D, 2, &data[4]))
        return -1;
    printf("mpu_set_accel_bias_6500_reg "
                   "(0x%x 0x%x 0x%x 0x%x 0x%x 0x%x)\n",
           data[0],data[1],data[2],data[3],data[4],data[5]);
    return 0;
}
...
//        printf("mpu_reset_fifo sensors %d\n",
//               st.chip_cfg.sensors);
    if (!(st.chip_cfg.sensors))
        return -1;

    data = 0;
    //int_enable->0x38  fifo_en--0x23 user_ctrl-- 0x6a
//    printf("st.reg->int_enable 0x%x"
//                   " st.reg->fifo_en 0x%x "
//                   "st.reg->user_ctrl 0x%x\n",
//           st.reg->int_enable,st.reg->fifo_en,st.reg->user_ctrl);

//    printf("3user cfg val 0x%x\n",data);
...
//    printf("st.chip_cfg.dmp_on is %d "
//                   "st.chip_cfg.int_enable %d\n",
//           st.chip_cfg.dmp_on,
//           st.chip_cfg.int_enable);

    if (st.chip_cfg.dmp_on) {
        data = BIT_FIFO_RST | BIT_DMP_RST;
//        printf("2user cfg val 0x%x\n",data);
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
            return -1;
        delay_ms(50);
...
//        printf("5user cfg val 0x%x\n",data);
        ...
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
//        printf("st.chip_cfg.fifo_enable is %d\n",
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
//    printf("set gyro cfg fsr %d 0x%x data 0x%x\n",fsr,
  //         st.reg->gyro_cfg,data);
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
//    printf("mpu_set_accel_fsr fsr %d "
//                   "st.reg->accel_cfg 0x%x "
//                   "data 0x%x\n",
//           fsr,st.reg->accel_cfg,data);
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
//    printf("lpf %d st.reg->lpf 0x%x data 0x%x\n",
  //         lpf,st.reg->lpf,data);
...
//    printf("\nst.chip_cfg.sensors %d "
//           "st.chip_cfg.dmp_on %d "
//           "st.chip_cfg.lp_accel_mode %d rate %d\n",
//           st.chip_cfg.sensors,st.chip_cfg.dmp_on,
...
#endif
//        printf("set div reg 0x%x data 0x%x rate %d "
//                       "st.chip_cfg.sample_rate %d\n",
//               st.reg->rate_div, data, rate, st.chip_cfg.sample_rate);
...
//    printf("mpu_configure_fifo sensors 0x%x "
//                   "st.chip_cfg.dmp_on %d"
//                   "st.chip_cfg.sensors 0x%x\n",
//           sensors,
//           st.chip_cfg.dmp_on,
//           st.chip_cfg.sensors);
...
//        printf("st.chip_cfg.fifo_enable is 0x%x "
//                       "sensors 0x%x "
//                       "st.chip_cfg.sensors 0x%x\n",
//               st.chip_cfg.fifo_enable,
//               sensors,
...
//        printf("st.chip_cfg.lp_accel_mode is %d result %d\n",
//               st.chip_cfg.lp_accel_mode,result);
...
//    printf("mpu_set_sensors sensors 0x%x\n",sensors);
    data = 0;
 ...
 *  @return     0 if successful.
 */
int mpu_read_fifo(short *gyro, short *accel, unsigned long long *timestamp,
...
//    log_i("FIFO count: %hd\n", fifo_count);
    ...
    get_ms((unsigned long long *)timestamp);
...
//    printf("st.chip_cfg.dmp_on %d st.chip_cfg.sensors 0x%x\n",
//           st.chip_cfg.dmp_on,st.chip_cfg.sensors);
...
//    printf("addr 0x%x st.reg->fifo_count_h 0x%x\n",
//           st.hw->addr,st.reg->fifo_count_h);
//    unsigned char dat = 0xe0;
//    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &dat))
//    {
//        printf("set user cfg 0xe0　fail\n");
//        return -1;
//    }
//    else
//    {
//        printf("set user cfg 0xe0　suc\n");
//    }
...
#ifdef ENABLE_DEBUG
    printf("fifo_count %d length %d (st.hw->max_fifo >> 1) %d\n",
           fifo_count,length,(st.hw->max_fifo >> 1));
#endif

    if (fifo_count < length) {
#ifdef ENABLE_ERROR
        printf("fifo_count less %d length %d\n",fifo_count,length);
#endif
        more[0] = 0;
        return -1;
    }
...
{
    unsigned char tmp;
//    printf("mpu_set_bypass bpass %d st.chip_cfg.bypass_mode %d\n",
//           bypass_on,st.chip_cfg.bypass_mode);
...
static int accel_self_test(int *bias_regular, int *bias_st)
{
    int jj, result = 0;
...
static int gyro_self_test(int *bias_regular, int *bias_st)
{
    int jj, result = 0;
    unsigned char tmp[3];
...
static int get_st_biases(int *gyro, int *accel, unsigned char hw_test)
{
    unsigned char data[MAX_PACKET_LENGTH];
    ...
    printf("2st.chip_cfg.fifo_enable is 0x%x\n",
           data[0]);
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        accel[0] += (int)accel_cur[0];
        accel[1] += (int)accel_cur[1];
        accel[2] += (int)accel_cur[2];
        gyro_cur[0] = (((short)data[6] << 8) | data[7]);
        gyro_cur[1] = (((short)data[8] << 8) | data[9]);
        gyro_cur[2] = (((short)data[10] << 8) | data[11]);
        gyro[0] += (int)gyro_cur[0];
        gyro[1] += (int)gyro_cur[1];
        gyro[2] += (int)gyro_cur[2];
    }
#ifdef EMPL_NO_64BIT
    gyro[0] = (int)(((float)gyro[0]*65536.f) / test.gyro_sens / packet_count);
    gyro[1] = (int)(((float)gyro[1]*65536.f) / test.gyro_sens / packet_count);
    gyro[2] = (int)(((float)gyro[2]*65536.f) / test.gyro_sens / packet_count);
    if (has_accel) {
        accel[0] = (int)(((float)accel[0]*65536.f) / test.accel_sens /
            packet_count);
        accel[1] = (int)(((float)accel[1]*65536.f) / test.accel_sens /
            packet_count);
        accel[2] = (int)(((float)accel[2]*65536.f) / test.accel_sens /
            packet_count);
        /* Don't remove gravity! */
        accel[2] -= 65536L;
    }
#else
    gyro[0] = (int)(((long long)gyro[0]<<16) / test.gyro_sens / packet_count);
    gyro[1] = (int)(((long long)gyro[1]<<16) / test.gyro_sens / packet_count);
    gyro[2] = (int)(((long long)gyro[2]<<16) / test.gyro_sens / packet_count);
    accel[0] = (int)(((long long)accel[0]<<16) / test.accel_sens /
        packet_count);
    accel[1] = (int)(((long long)accel[1]<<16) / test.accel_sens /
        packet_count);
    accel[2] = (int)(((long long)accel[2]<<16) / test.accel_sens /
        packet_count);
    /* Don't remove gravity! */
    if (accel[2] > 0L)
        accel[2] -= 65536L;
...
static int accel_6500_self_test(int *bias_regular, int *bias_st, int debug)
{
...
static int gyro_6500_self_test(int *bias_regular, int *bias_st, int debug)
{
    int i, result = 0, otp_value_zero = 0;
...
static int get_st_6500_biases(int *gyro, int *accel, unsigned char hw_test, int debug)
{
...
//    printf("test.wait_ms is %d\n",test.wait_ms);
    //test.wait_ms is 200ms
  ...
  //  printf("fifo en 0x%x\n",data[0]);
    ...
    //stop FIFO
    data[0] = 0;
//    printf("5fifo en 0x%x\n",data[0]);
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data))
        return -1;

    gyro[0] = (int)(((long long)gyro[0]<<16) / test.gyro_sens / s);
    gyro[1] = (int)(((long long)gyro[1]<<16) / test.gyro_sens / s);
    gyro[2] = (int)(((long long)gyro[2]<<16) / test.gyro_sens / s);
    accel[0] = (int)(((long long)accel[0]<<16) / test.accel_sens / s);
    accel[1] = (int)(((long long)accel[1]<<16) / test.accel_sens / s);
    accel[2] = (int)(((long long)accel[2]<<16) / test.accel_sens / s);
 ...
 */
int mpu_run_6500_self_test(int *gyro, int *accel, unsigned char debug)
{
    const unsigned char tries = 2;
    int gyro_st[3], accel_st[3];
    ...
    printf("mpu_run_6500_self_test start\n");
    if (st.chip_cfg.dmp_on) {
        mpu_set_dmp_state(0);
        dmp_was_on = 1;
    } else
    ...
    printf("mpu_run_6500_self_test start 2\n");

    if(debug)
    	log_i("Retrieving Biases\r\n");

    printf("mpu_run_6500_self_test start 3\n");
    for (ii = 0; ii < tries; ii++)
        if (!get_st_6500_biases(gyro, accel, 0, debug))
            break;
    printf("mpu_run_6500_self_test start ii %d tries %d\n",ii,tries);
    if (ii == tries) {
    ...
    if (!accel_result)
        result |= 0x02;
#if 1
    printf("force compass test 0x%x\n",0x04);
    result |= 0x04;
#else
#ifdef AK89xx_SECONDARY
 ...
 *  @return     Result mask (see above).
 */
int mpu_run_self_test(int *gyro, int *accel)
{
#ifdef MPU6050
    const unsigned char tries = 2;
    int gyro_st[3], accel_st[3];
...
#define LOAD_CHUNK  (16)
    ...
    {
        printf("mpu_load_firmware error a\n");
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        {
            printf("mpu_load_firmware error b\n");
            return -1;
        }

        if (mpu_read_mem(ii, this_write, cur))
        {
            printf("mpu_load_firmware error c\n");
            return -1;
        }
        if (memcmp(firmware+ii, cur, this_write))
        {
            printf("mpu_load_firmware error e\n");
            return -2;
        }
    }
    ...
    {
        printf("mpu_load_firmware error f\n");
        return -1;
    }
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        printf("set st.chip_cfg.dmp_sample_rate %d\n",
               st.chip_cfg.dmp_sample_rate);
...
static int setup_compass(void)
{
    ...
    for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
        ...
        if(result == 0)
        {
            printf("compass addr 0x%x data 0x%x\n",akm_addr,data[0]);
        }
    ...
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
    st.chip_cfg.mag_sens_adj[0] = (int)data[0] + 128;
    st.chip_cfg.mag_sens_adj[1] = (int)data[1] + 128;
    st.chip_cfg.mag_sens_adj[2] = (int)data[2] + 128;
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(short *data, unsigned long long *timestamp)
{
#ifdef AK89xx_SECONDARY
    unsigned char tmp[9];
    ...
    data[0] = ((int)data[0] * st.chip_cfg.mag_sens_adj[0]) >> 8;
    data[1] = ((int)data[1] * st.chip_cfg.mag_sens_adj[1]) >> 8;
    data[2] = ((int)data[2] * st.chip_cfg.mag_sens_adj[2]) >> 8;
