...
*/
void inv_set_accel_bias(const int *bias, int accuracy)
{
    if (bias) {
...
*/
void inv_set_gyro_bias(const int *bias, int accuracy)
{
    if (bias != NULL) {
 ...
 */
void inv_get_gyro_bias(int *bias, int *temp)
{
    if (bias != NULL)
...
*/
void inv_get_accel_bias(int *bias, int *temp)
{
    if (bias != NULL)
...
* @return Returns INV_SUCCESS if successful or an error code if not.
 */
inv_error_t inv_build_temp(const int temp, inv_time_t timestamp)
{
#ifdef INV_PLAYBACK_DBG
    if (inv_data_builder.debug_mode == RD_RECORD) {
...
//    printf("mode and sensor status 0x%x 0x%x\n",
//           mode,sensors.temp.status);
...
{
    if (data != NULL) {
//        printf("get accel %d %d %d\n",sensors.accel.calibrated[0],sensors.accel.calibrated[1],sensors.accel.calibrated[2]);
