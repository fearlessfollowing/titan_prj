...
static void print_event(int type, int code, int value, int print_flags)
{
    ...
    if (print_flags & PRINT_LABELS) {
        ...
        printf("get event %04x %04x %08x", type, code, value);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        printf("333poll %d, returned %d nfds %d "
                       "ufds[0].revents 0x%x\n",
               nfds, pollres, nfds, ufds[0].revents);
        if(ufds[0].revents & POLLIN) {
            read_notify(device_path, ufds[0].fd, print_flags);
        }
        for(i = 1; i < nfds; i++) {
            printf("rec event[%d] 0x%x\n",i,ufds[i].revents);
            if(ufds[i].revents) {
