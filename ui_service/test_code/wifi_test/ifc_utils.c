...
int ifc_init(void)
{
    ...
    if (DBG) printf("ifc_init_returning %d\n", ret);
    return ret;
}
...
int ifc_set_addr(const char *name, in_addr_t addr)
{
    ...
    if (DBG) printf("ifc_set_addr(%s, xx) = %d\n", name, ret);
    return ret;
}
...
int ifc_clear_ipv6_addresses(const char *name) {
    ...
    while (fscanf(f, "%32s %*02x %02x %*02x %*02x %63s\n",
        ...
        if (ret) {
            ALOGE("Deleting address %s/%d on %s: %s\n", addrstr, prefixlen, ifname,
                 strerror(-ret));
            lasterror = ret;
        }
    }
...
int ifc_set_mask(const char *name, in_addr_t mask)
{
    ...
    if (DBG) printf("ifc_set_mask(%s, xx) = %d\n", name, ret);
    return ret;
}
...
int ifc_create_default_route(const char *name, in_addr_t gw)
{
    ...
    if (DBG) printf("ifc_create_default_route(%s, %d) = %d\n", name, gw, ret);
    return ret;
}
...
int ifc_remove_default_route(const char *ifname)
{
    ...
    if ((result = ioctl(ifc_ctl_sock, SIOCDELRT, &rt)) < 0) {
        ALOGD("failed to remove default route for %s: %s\n", ifname, strerror(errno));
    }
    ifc_close();
    return result;
}
...
ifc_configure(const char *ifname,
    ...
    if (ifc_set_prefixLength(ifname, prefixLength)) {
        printf("failed to set prefixLength %d: %s\n", prefixLength, strerror(errno));
        ifc_close();
        return -1;
    }
