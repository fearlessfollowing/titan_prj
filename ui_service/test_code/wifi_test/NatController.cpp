...
int NatController::enableNat(const char* intIface, const char* extIface) {
    ...
    if (!strcmp(intIface, extIface)) {
        printf("Duplicate interface specified: %s %s", intIface, extIface);
        errno = EINVAL;
        return -1;
    }
    printf("0 enableNat(intIface=<%s>, extIface=<%s>) natCount %d\n",intIface, extIface,natCount);
    ...
    if (natCount == 0) {
        ...
        if (runCmd(ARRAY_SIZE(cmd), cmd)) {
            printf("Error setting postroute rule: iface=%s\n", extIface);
            // unwind what's been done, but don't care about success - what more could we do?
            setDefaults();
            return -1;
        }
    }
    printf("2enableNat(intIface=<%s>, extIface=<%s>) natCount %d\n",intIface, extIface,natCount);
    if (setForwardRules(true, intIface, extIface) != 0) {
        printf("Error setting forward rules\n");
        if (natCount == 0) {
            setDefaults();
        }
        errno = ENODEV;
    ...
    printf("3enableNat(intIface=<%s>, extIface=<%s>) natCount %d\n",intIface, extIface,natCount);
    ...
    printf("5enableNat(intIface=<%s>, extIface=<%s>) natCount %d\n",intIface, extIface,natCount);
    const char *cmd2[] = {
            IPTABLES_PATH,
            "-w",
            "-A",
