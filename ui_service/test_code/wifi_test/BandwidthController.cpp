...
int BandwidthController::runIpxtablesCmd(const char *cmd, IptJumpOp jumpHandling,
    ...
    printf("runIpxtablesCmd(cmd=%s)", cmd);
...
int BandwidthController::runIptablesCmd(const char *cmd, IptJumpOp jumpHandling,
    ...
    if (StrncpyAndCheck(buffer, fullCmd.c_str(), sizeof(buffer))) {
        printf("iptables command too long");
        return -1;
    }
    ...
    while ((tmp = strsep(&next, " "))) {
        ...
        if (argc >= MAX_CMD_ARGS) {
            printf("iptables argument overflow");
            return -1;
        }
    }
    ...
    if (res && failureHandling == IptFailShow) {
      printf("runIptablesCmd(): res=%d status=%d failed %s", res, status,
            fullCmd.c_str());
    }
    return res;
}
...
int BandwidthController::runCommands(int numCommands, const char *commands[],
    ...
    printf("runCommands(): %d commands", numCommands);
...
std::string BandwidthController::makeIptablesSpecialAppCmd(IptOp op, int uid, const char *chain) {
    ...
    case IptOpInsert:
        ...
        break;
    case IptOpAppend:
        printf("Append op not supported for %s uids", chain);
        res = "";
        return res;
        break;
    case IptOpReplace:
...
int BandwidthController::manipulateSpecialApps(int numUids, char *appStrUids[],
    ...
    case SpecialAppOpRemove:
        ...
        break;
    default:
        printf("Unexpected app Op %d", appOp);
        return -1;
    }
    ...
    for (uidNum = 0; uidNum < numUids; uidNum++) {
        ...
        if (*end || !*appStrUids[uidNum]) {
            printf(failLogTemplate, appStrUids[uidNum], appUids[uidNum], chain);
            goto fail_parse;
        }
    }
    ...
    for (uidNum = 0; uidNum < numUids; uidNum++) {
        ...
        if (appOp == SpecialAppOpRemove) {
            if (!found) {
                printf("No such appUid %d to remove", uid);
                return -1;
            }
            specialAppUids.erase(it);
        } else {
            if (found) {
                printf("appUid %d exists already", uid);
                return -1;
            }
        ...
        if (runIpxtablesCmd(iptCmd.c_str(), jumpHandling)) {
            printf(failLogTemplate, appStrUids[uidNum], uid, chain);
...
std::string BandwidthController::makeIptablesQuotaCmd(IptOp op, const char *costName, int64_t quota) {
    ...
    printf("makeIptablesQuotaCmd(%d, %" PRId64")", op, quota);
...
int BandwidthController::prepCostlyIface(const char *ifn, QuotaType quotaType) {
    ...
    case QuotaShared:
        ...
        break;
    default:
        printf("Unexpected quotatype %d", quotaType);
        return -1;
    }
...
int BandwidthController::setInterfaceSharedQuota(const char *iface, int64_t maxBytes) {
    ...
    if (!maxBytes) {
        ...
        printf("Invalid bytes value. 1..max_int64.");
        return -1;
    }
    if (!isIfaceName(iface))
        return -1;
    if (StrncpyAndCheck(ifn, iface, sizeof(ifn))) {
        printf("Interface name longer than %d", MAX_IFACENAME_LEN);
        return -1;
    }
    ifaceName = ifn;
    ...
    if (maxBytes != sharedQuotaBytes) {
        ...
        if (res) {
            printf("Failed update quota for %s", costName);
            goto fail;
        }
        sharedQuotaBytes = maxBytes;
    }
...
int BandwidthController::removeInterfaceSharedQuota(const char *iface) {
    ...
    if (it == sharedQuotaIfaces.end()) {
        printf("No such iface %s to delete", ifn);
        return -1;
    }
...
int BandwidthController::setInterfaceQuota(const char *iface, int64_t maxBytes) {
    ...
    if (it == quotaIfaces.end()) {
        ...
        res |= updateQuota(costName, maxBytes);
        if (res) {
            printf("Failed update quota for %s", iface);
            goto fail;
        }
        it->quota = maxBytes;
    }
...
int BandwidthController::getInterfaceQuota(const char *costName, int64_t *bytes) {
    ...
    free(fname);
    if (!fp) {
        printf("Reading quota %s failed (%s)", costName, strerror(errno));
        return -1;
    }
    scanRes = fscanf(fp, "%" SCNd64, bytes);
    printf("Read quota res=%d bytes=%" PRId64, scanRes, *bytes);
    fclose(fp);
...
int BandwidthController::updateQuota(const char *quotaName, int64_t bytes) {
    ...
    if (!isIfaceName(quotaName)) {
        printf("updateQuota: Invalid quotaName \"%s\"", quotaName);
        return -1;
    }
    ...
    free(fname);
    if (!fp) {
        printf("Updating quota %s failed (%s)", quotaName, strerror(errno));
        return -1;
    }
    fprintf(fp, "%" PRId64"\n", bytes);
    fclose(fp);
...
int BandwidthController::setGlobalAlert(int64_t bytes) {
    ...
    if (globalAlertBytes) {
        ...
        if (globalAlertTetherCount) {
            printf("setGlobalAlert for %d tether", globalAlertTetherCount);
...
int BandwidthController::setGlobalAlertInForwardChain(void) {
    ...
    printf("setGlobalAlertInForwardChain(): %d tether",
           globalAlertTetherCount);
...
int BandwidthController::setSharedAlert(int64_t bytes) {
    if (!sharedQuotaBytes) {
        printf("Need to have a prior shared quota set to set an alert");
        return -1;
    }
    if (!bytes) {
        printf("Invalid bytes value. 1..max_int64.");
...
int BandwidthController::setInterfaceAlert(const char *iface, int64_t bytes) {
    ...
    if (!isIfaceName(iface)) {
        printf("setInterfaceAlert: Invalid iface \"%s\"", iface);
        return -1;
    }
    ...
    if (it == quotaIfaces.end()) {
        printf("Need to have a prior interface quota set to set an alert");
        return -1;
    }
...
int BandwidthController::removeInterfaceAlert(const char *iface) {
    ...
    if (!isIfaceName(iface)) {
        printf("removeInterfaceAlert: Invalid iface \"%s\"", iface);
        return -1;
    }
    ...
    if (it == quotaIfaces.end()) {
        printf("No prior alert set for interface %s\n", iface);
        return -1;
    }
...
int BandwidthController::setCostlyAlert(const char *costName, int64_t bytes, int64_t *alertBytes) {
    ...
    if (!isIfaceName(costName)) {
        printf("setCostlyAlert: Invalid costName \"%s\"", costName);
        return -1;
    }
...
int BandwidthController::removeCostlyAlert(const char *costName, int64_t *alertBytes) {
    ...
    if (!isIfaceName(costName)) {
        printf("removeCostlyAlert: Invalid costName \"%s\"", costName);
        return -1;
    }

    if (!*alertBytes) {
        printf("No prior alert set for %s alert\n", costName);
        return -1;
    }
 ...
 */
#if 1
int BandwidthController::parseForwardChainStats(const TetherStats filter,
...
//        printf("parse res=%d iface0=<%s> iface1=<%s> pkts=%" PRId64" bytes=%" PRId64" rest=<%s> orig line=<%s>", res,
//             iface0, iface1, packets, bytes, rest, buffPtr);
        ...
        if (filter.intIface[0] && filter.extIface[0]) {
            if (filter.intIface == iface0 && filter.extIface == iface1) {
                printf("2Filter RX iface_in=%s iface_out=%s rx_bytes=%" PRId64" rx_packets=%" PRId64" ", iface0, iface1, bytes, packets);
                stats.rxPackets = packets;
                stats.rxBytes = bytes;
            } else if (filter.intIface == iface1 && filter.extIface == iface0) {
                printf("2Filter TX iface_in=%s iface_out=%s rx_bytes=%" PRId64" rx_packets=%" PRId64" ", iface0, iface1, bytes, packets);
                stats.txPackets = packets;
                stats.txBytes = bytes;
            }
        } else if (filter.intIface[0] || filter.extIface[0]) {
            if (filter.intIface == iface0 || filter.extIface == iface1) {
                printf("1Filter RX iface_in=%s iface_out=%s rx_bytes=%" PRId64" rx_packets=%" PRId64" ", iface0, iface1, bytes, packets);
                stats.intIface = iface0;
                stats.extIface = iface1;
                stats.rxPackets = packets;
                stats.rxBytes = bytes;
            } else if (filter.intIface == iface1 || filter.extIface == iface0) {
                printf("1Filter TX iface_in=%s iface_out=%s rx_bytes=%" PRId64" rx_packets=%" PRId64" ", iface0, iface1, bytes, packets);
            ...
            if (!stats.intIface[0]) {
                printf("0Filter RX iface_in=%s iface_out=%s rx_bytes=%" PRId64" rx_packets=%" PRId64" ", iface0, iface1, bytes, packets);
                stats.intIface = iface0;
                stats.extIface = iface1;
                stats.rxPackets = packets;
                stats.rxBytes = bytes;
            } else if (stats.intIface == iface1 && stats.extIface == iface0) {
                printf("0Filter TX iface_in=%s iface_out=%s rx_bytes=%" PRId64" rx_packets=%" PRId64" ", iface0, iface1, bytes, packets);
                stats.txPackets = packets;
                stats.txBytes = bytes;
            }
        }
        if (stats.rxBytes != -1 && stats.txBytes != -1) {
            printf("rx_bytes=%" PRId64" tx_bytes=%" PRId64" filterPair=%d", stats.rxBytes, stats.txBytes, filterPair);
...
int BandwidthController::getTetherStats( TetherStats &stats, std::string &extraProcessingInfo) {
    ...
    if (!iptOutput) {
            printf("Failed to run %s err=%s", fullCmd.c_str(), strerror(errno));
...
//        printf("parse res=%d costly=<%s> orig line=<%s>", res,
