...
uint32_t getRouteTableForInterface(const char* interface) {
    ...
    if (index) {
        ...
        // Check if this is an alias address and add another offset to not conflict with main interface's table.
        // use the alias number (after the ":" in the name) to generate a unique offset for each alias.
        char *alias=strstr(interface, ":");
        if (alias) {
           index +=  (1+atoi(alias+1)) * RouteController::ROUTE_TABLE_OFFSET_FOR_ALIAS;
        }
    ...
    if (iter == interfaceToTable.end()) {
        printf("cannot find interface %s", interface);
...
void updateTableNamesFile() {
    ...
    if (!WriteStringToFile(contents, RT_TABLES_PATH, RT_TABLES_MODE, AID_SYSTEM, AID_WIFI)) {
        printf("failed to write to %s (%s)", RT_TABLES_PATH, strerror(errno));
        return;
    }
}
...
WARN_UNUSED_RESULT int sendNetlinkRequest(uint16_t action, uint16_t flags, iovec* iov, int iovlen) {
    ...
    printf("sendNetlinkRequest %d %d %d %d %d\n",nlmsg.nlmsg_len,	/* Length of message including header */
           nlmsg.nlmsg_type,	/* Message content */
    nlmsg.nlmsg_flags,	/* Additional flags */
    nlmsg.nlmsg_seq,	/* Sequence number */
    nlmsg.nlmsg_pid	/* Sending process port ID */);
    ...
    printf("sendNetlinkRequest (%d 0x%x %d)\n",action,flags,iovlen);
    if (sock != -1 &&
            ...
            printf("sendNetlinkRequest c %d %zd\n",ret,sizeof(response));
        if (ret == sizeof(response)) {
            ret = response.err.error;  // Netlink errors are negative errno.
            if (ret) {
                printf("netlink response contains error (%s)", strerror(-ret));
            }
        } else {
            printf("bad netlink response message size (%d != %zu)", ret, sizeof(response));
            ret = -EBADMSG;
        }
    } else {
        printf("netlink socket/connect/writev/recv failed (%s)", strerror(errno));
        ret = -errno;
    }
    printf("sendNetlinkRequest e\n");
    if (sock != -1) {
        close(sock);
    }
    printf("sendNetlinkRequest f ret %d\n",ret);
#endif
    return ret;
}
...
int padInterfaceName(const char* input, char* name, size_t* length, uint16_t* padding) {
    ...
    *length = strlcpy(name, input, IFNAMSIZ) + 1;
    if (*length > IFNAMSIZ) {
        printf("interface name too long (%zu > %u)", *length, IFNAMSIZ);
        return -ENAMETOOLONG;
    }
...
WARN_UNUSED_RESULT int modifyIpRule(uint16_t action, uint32_t priority, uint32_t table,
    ...
    if (fwmark & ~mask) {
        printf("mask 0x%x does not select all the bits set in fwmark 0x%x", mask, fwmark);
        return -ERANGE;
    }
    printf("modifyIpRule "
                   "(%d 0x%x 0x%x 0x%x 0x%x %s %s)\n",
           action,priority, table,fwmark,mask,
           iif,oif);
    ...
    if ((uidStart == INVALID_UID) != (uidEnd == INVALID_UID)) {
        printf("incompatible start and end UIDs (%u vs %u)", uidStart, uidEnd);
        return -EUSERS;
    }
    ...
    memset(&rule,0,sizeof(fib_rule_hdr));
    ...
    if (table == RT_TABLE_UNSPEC && rule.action == FR_ACT_TO_TBL && action != RTM_DELRULE) {
        printf("RT_TABLE_UNSPEC only allowed when deleting rules");
        return -ENOTUNIQ;
    }
...
WARN_UNUSED_RESULT int modifyIpRoute(uint16_t action, uint32_t table, const char* interface,
    ...
    printf("modifyIpRule (%d %d %s %s %s)\n",
           action,table,interface,destination,nexthop);
    // Parse the prefix.
    ...
    int rawLength = parsePrefix(destination, &family, rawAddress, sizeof(rawAddress),
                                &prefixLength);
    if (rawLength < 0) {
        printf("parsePrefix failed for destination %s (%s)", destination, strerror(-rawLength));
        return rawLength;
    }

    if (static_cast<size_t>(rawLength) > sizeof(rawAddress)) {
        printf("impossible! address too long (%d vs %zu)", rawLength, sizeof(rawAddress));
    ...
    if (nexthop && !strcmp(nexthop, "unreachable")) {
        ...
        if (nexthop && inet_pton(family, nexthop, rawNexthop) <= 0) {
            printf("inet_pton failed for nexthop %s", nexthop);
            return -EINVAL;
        }
    }
    ...
    // If the same route exists with an "old" nexthop then the operation will fail.
    // So to avoid failure don't specify nexthop in the request.
    if (nexthop && (action == RTM_DELROUTE))
    {
        nexthop = NULL;
    }
...
WARN_UNUSED_RESULT int modifyIncomingPacketMark(unsigned netId, const char* interface,
    ...
    if (execIptables(V4V6, "-t", "mangle", add ? "-A" : "-D", "INPUT", "-i", interface, "-j",
        ...
        printf("failed to change iptables rule that sets incoming packet mark");
        return -EREMOTEIO;
    }
...
int configureDummyNetwork() {
    ...
    ifc_close();
    if (ret) {
        printf("Can't bring up %s: %s", interface, strerror(errno));
        return -errno;
    }

    if ((ret = modifyOutputInterfaceRules(interface, table, PERMISSION_NONE,
                                          INVALID_UID, INVALID_UID, ACTION_ADD))) {
        printf("Can't create oif rules for %s: %s", interface, strerror(-ret));
        return ret;
    }

    if ((ret = modifyIpRoute(RTM_NEWROUTE, table, interface, "0.0.0.0/0", NULL))) {
        printf("Can't add IPv4 default route to %s: %s", interface, strerror(-ret));
        return ret;
    }

    if ((ret = modifyIpRoute(RTM_NEWROUTE, table, interface, "::/0", NULL))) {
        printf("Can't add IPv6 default route to %s: %s", interface, strerror(-ret));
        return ret;
    }
...
WARN_UNUSED_RESULT int modifyRoute(uint16_t action, const char* interface, const char* destination,
    ...
    // For legacy_system table consider that the route we are trying to setup is the correct one.
    // Then remove this one.
    // nextHop may be different than the one for the route already present (this has been observed when sending several MMS)
    if (ret==-EEXIST && tableType == RouteController::LEGACY_SYSTEM && action == RTM_NEWROUTE) {
        // delete the existing old route.
        ret = modifyIpRoute(RTM_DELROUTE, table, interface, destination, nexthop);
        // try again to add the route we want.
        ret = modifyIpRoute(RTM_NEWROUTE, table, interface, destination, nexthop);
    }
...
WARN_UNUSED_RESULT int flushRoutes(const char* interface) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(IP_VERSIONS); ++i) {
        ...
        if (err) {
            printf("failed to flush %s routes in table %s after %d attempts",
