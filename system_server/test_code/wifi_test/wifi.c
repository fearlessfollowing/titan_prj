...
#ifndef WIFI_DRIVER_OP_MODE_PARAM
#define WIFI_DRIVER_OP_MODE_PARAM	"/sys/module/wlan/parameters/op_mode"
#endif
...
int wifi_change_driver_state(const char *state)
{
    ...
    if (!state)
        return -1;
    printf("wifi_change_driver_state state %s WIFI_DRIVER_STATE_CTRL_PARAM %s",
    state,WIFI_DRIVER_STATE_CTRL_PARAM);
    fd = TEMP_FAILURE_RETRY(open(WIFI_DRIVER_STATE_CTRL_PARAM, O_WRONLY));
    if (fd < 0) {
        printf("Failed to open driver state control param (%s)", strerror(errno));
        return -1;
    }
    len = strlen(state) + 1;
    if (TEMP_FAILURE_RETRY(write(fd, state, len)) != len) {
        printf("Failed to write driver state control param (%s)", strerror(errno));
        ret = -1;
    }
    close(fd);
    return ret;
...
int is_wifi_driver_loaded() {
   ...
   ALOGW("is_wifi_driver_loaded MODULE_FILE %s", MODULE_FILE);
...
int wifi_load_driver()
{
    ...
    printf("wifi_load_driver a DRIVER_MODULE_PATH %s DRIVER_MODULE_ARG %s FIRMWARE_LOADER %s FIRMWARE_LOADER %s",
        DRIVER_MODULE_PATH,DRIVER_MODULE_ARG,FIRMWARE_LOADER,FIRMWARE_LOADER);
...
#else
#ifdef WIFI_DRIVER_STATE_CTRL_PARAM
    printf("wifi_load_driver b");
    if (is_wifi_driver_loaded()) {
        return 0;
    }
    printf("wifi_load_driver b2");
    if (wifi_change_driver_state(WIFI_DRIVER_STATE_ON) < 0)
        return -1;
#endif
    printf("wifi_load_driver c");
    property_set(DRIVER_PROP_NAME, "ok");
    return 0;
#endif
}
...
int wifi_connect_on_socket_path(const char *path)
{
    ...
    if (!property_get(supplicant_prop_name, supp_status, NULL)
            || strcmp(supp_status, "running") != 0) {
        printf("Supplicant not running, cannot connect");
        return -1;
    }
...
int wifi_change_fw_path(const char *fwpath)
{
    ...
    printf("wifi_change_fw_path (%s %s %d)",WIFI_DRIVER_FW_PATH_PARAM,fwpath,len);
    close(fd);
    return ret;
}
...
int wifi_change_op_mode(int op_mode)
{
    ...
    if (fd < 0) {
        printf("Failed to open wlan operation mode param (%s)", strerror(errno));
        return -1;
    }
    ...
    printf("wifi_change_op_mode (%s %d %d)",WIFI_DRIVER_OP_MODE_PARAM,op_mode,len);
    close(fd);
    return ret;
}
