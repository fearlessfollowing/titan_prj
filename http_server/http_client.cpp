...
void HttpClient::SendReq(const std::string &url, 
                                ReqCallback req_callback, const char* extra_headers, const char* post_data)
{
	s_req_callback = req_callback;
	mg_mgr mgr;
	mg_mgr_init(&mgr, NULL);
	
    auto connection = mg_connect_http(&mgr, OnHttpEvent, url.c_str(), extra_headers, post_data);
