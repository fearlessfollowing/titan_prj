...
#define PROP_MAX_LOG_FILE_SIZE  "sys.log_max_size"


#ifndef LOG_FILE_DEFAULT_SIZE
#define LOG_FILE_DEFAULT_SIZE    30     /* 默认的日志文件最大为20MB */
#endif
...
#define MIN_LOG_FILE_SIZE   (10*1024*1024)
...
LogWrapper::LogWrapper(std::string path, std::string name, bool bSave2File, bool bSendToLogd)
{
    ...
    if (true == bSave2File) {
        ...
        if (pLogFileSize) {
            mLogFileMaxSize = atoi(pLogFileSize);
            mLogFileMaxSize *= (1024*1024);
        } else {
            mLogFileMaxSize = LOG_FILE_DEFAULT_SIZE * 1024 * 1024;
        }

        if (mLogFileMaxSize < MIN_LOG_FILE_SIZE)
            mLogFileMaxSize = LOG_FILE_DEFAULT_SIZE * 1024 * 1024;
...
{	
	static char LogLevelString[][10] = {"UNKOWN", "DEFAULT", "VERBOSE", "INFO ", "DEBUG", "WARN", "ERROR", "FATAL"};
	...
	if (mLogFileHandle) {
		fprintf(mLogFileHandle, "%04d-%02d-%02d %02d:%02d:%02d:%06ld %s %s [%s:%d]",
			tmt->tm_year + 1900,
			tmt->tm_mon + 1,
			tmt->tm_mday,
			tmt->tm_hour,
