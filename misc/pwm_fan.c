...
struct fan_dev_data {
	...
	int                         fan_pwm_max;
	struct device *             dev;
	int                         tach_gpio;          /* 转速计检测GPIO */
	int                         tach_irq;           /* 转速计检测中断 */
	atomic_t                    tach_enabled;
	int                         fan_state_cap;
	int                         pwm_gpio;
	int                         pwm_id;
	const char *                name;
	struct regulator*           fan_reg;            /* 风扇的电源控制 */
	bool                        is_fan_reg_enabled; /* 风扇的供电是否开启 */
	struct dentry*              debugfs_root;       /* debugfs中入口 */

	/* for tach feedback */
	atomic64_t                  rpm_measured;       /* 风扇转速测量 */
...
DEFINE_SIMPLE_ATTRIBUTE(fan_cap_pwm_fops, fan_cap_pwm_show, fan_cap_pwm_set, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(fan_temp_control_fops, fan_temp_control_show, fan_temp_control_set, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(fan_target_pwm_fops, fan_target_pwm_show, fan_target_pwm_set, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(fan_cur_pwm_fops, fan_cur_pwm_show, NULL, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(fan_step_time_fops, fan_step_time_show, fan_step_time_set, "%llu\n");
...
static int pwm_fan_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *cur_state)
{
...
static int pwm_fan_set_cur_state(struct thermal_cooling_device *cdev, unsigned long cur_state)
{
	struct fan_dev_data *fan_data = cdev->devdata;
	int target_pwm;
...
static int pwm_fan_get_max_state(struct thermal_cooling_device *cdev, unsigned long *max_state)
{
	struct fan_dev_data *fan_data = cdev->devdata;
...
static int fan_get_rru(int pwm, struct fan_dev_data *fan_data)
{
	...
	for (i = 0; i < fan_data->active_steps - 1 ; i++) {
		if ((pwm >= fan_data->fan_pwm[i]) && (pwm < fan_data->fan_pwm[i + 1])) {
...
static void set_pwm_duty_cycle(int pwm, struct fan_dev_data *fan_data)
{
	...
	if (fan_data != NULL && fan_data->pwm_dev != NULL) {
		if (pwm == 0)
			...
			duty = (fan_data->fan_pwm_max - pwm) * fan_data->precision_multiplier;
		
        pwm_config(fan_data->pwm_dev, duty, fan_data->pwm_period);
...
static void fan_ramping_work_func(struct work_struct *work)
{
	...
	struct fan_dev_data *fan_data = container_of(dwork, struct fan_dev_data, fan_ramp_work);
	...
	if (fan_data->next_target_pwm > fan_data->fan_cur_pwm) {
		...
		next_pwm = min(get_next_higher_pwm(cur_pwm, fan_data), fan_data->fan_cur_pwm);
		...
		next_pwm = max(get_next_lower_pwm(cur_pwm, fan_data), fan_data->fan_cur_pwm);
	...
	if ((next_pwm != 0) && !(fan_data->is_fan_reg_enabled)) {
		...
		if (err < 0)
			dev_err(fan_data->dev, " Coudn't enable vdd-fan\n");
		else {
	...
	if ((next_pwm == 0) && (fan_data->is_fan_reg_enabled)) {
		...
		if (err < 0)
			dev_err(fan_data->dev, " Couldn't disable vdd-fan\n");
		else {
			dev_dbg(fan_data->dev, " Disabled vdd-fan\n");
...
static void fan_tach_work_func(struct work_struct *work)
{
	...
	struct fan_dev_data *fan_data = container_of(dwork, struct fan_dev_data, fan_tach_work);
	...
	if (tach_pulse_count <= 1) {
		...
		dev_err(fan_data->dev, "invalid irq time diff: caught %d, diff_us %lld\n", tach_pulse_count, diff_us);
	} else {
	...
	queue_delayed_work(fan_data->tach_workqueue, &(fan_data->fan_tach_work), msecs_to_jiffies(fan_data->tach_period));
}

static ssize_t show_fan_pwm_cap_sysfs(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t set_fan_pwm_cap_sysfs(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
	long val;
	int ret;
...
static ssize_t show_fan_state_cap_sysfs(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
	int ret;
...
static ssize_t set_fan_state_cap_sysfs(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
	long val;
	int ret;
	...
	fan_data->fan_cap_pwm = fan_data->fan_pwm[fan_data->fan_state_cap_lookup[val]];
	target_pwm = min(fan_data->fan_cap_pwm, fan_data->next_target_pwm);
	fan_update_target_pwm(fan_data, target_pwm);
	mutex_unlock(&fan_data->fan_state_lock);
	return count;
...
static ssize_t set_fan_pwm_state_map_sysfs(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
...
static ssize_t show_tach_enabled_sysfs(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
	int ret;
	int val;
...
static ssize_t set_tach_enabled_sysfs(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
	int val, ret;
	int tach_enabled;
 ...
 */
static ssize_t show_rpm_measured_sysfs(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct fan_dev_data *fan_data = dev_get_drvdata(dev);
	int ret;
	u64 val;
...
static DEVICE_ATTR(pwm_cap, S_IWUSR | S_IRUGO, show_fan_pwm_cap_sysfs, set_fan_pwm_cap_sysfs);
static DEVICE_ATTR(state_cap, S_IWUSR | S_IRUGO, show_fan_state_cap_sysfs, set_fan_state_cap_sysfs);
static DEVICE_ATTR(pwm_state_map, S_IWUSR | S_IRUGO, NULL, set_fan_pwm_state_map_sysfs);
static DEVICE_ATTR(tach_enabled, S_IWUSR | S_IRUGO, show_tach_enabled_sysfs, set_tach_enabled_sysfs);
...
irqreturn_t fan_tach_isr(int irq, void *data)
{
	...
	if (fan_data->irq_count == 0)   /* 记录第一次产生中断的时间戳 */
...
/*
 * RPM(Revolutions Per Minute): 风扇每分钟转速
 * 
 */
...
static int pwm_fan_probe(struct platform_device *pdev)
{
	...
	fan_data = devm_kzalloc(&pdev->dev, sizeof(struct fan_dev_data), GFP_KERNEL);
	if (!fan_data)
		return -ENOMEM;
	...
	/* rpm array */
	rpm_data = devm_kzalloc(&pdev->dev, sizeof(int) * (fan_data->active_steps), GFP_KERNEL);
	if (!rpm_data) {
		err = -ENOMEM;
		goto rpm_alloc_fail;
	}

	of_err |= of_property_read_u32_array(data_node, "active_rpm", rpm_data, (size_t) fan_data->active_steps);
	fan_data->fan_rpm = rpm_data;

	/* rru array */
	rru_data = devm_kzalloc(&pdev->dev, sizeof(int) * (fan_data->active_steps), GFP_KERNEL);
	if (!rru_data) {
		err = -ENOMEM;
		goto rru_alloc_fail;
	}
	...
	/* rrd array */
	rrd_data = devm_kzalloc(&pdev->dev, sizeof(int) * (fan_data->active_steps), GFP_KERNEL);
	if (!rrd_data) {
		err = -ENOMEM;
		goto rrd_alloc_fail;
	}
    
	of_err |= of_property_read_u32_array(data_node, "active_rrd", rrd_data, (size_t) fan_data->active_steps);
	
    fan_data->fan_rrd = rrd_data;

	/* state_cap_lookup array */
	lookup_data = devm_kzalloc(&pdev->dev, sizeof(int) * (fan_data->active_steps), GFP_KERNEL);
	if (!lookup_data) {
		err = -ENOMEM;
		goto lookup_alloc_fail;
	}

	of_err |= of_property_read_u32_array(data_node, "state_cap_lookup", lookup_data, (size_t) fan_data->active_steps);
	fan_data->fan_state_cap_lookup = lookup_data;

	/* pwm array */
	pwm_data = devm_kzalloc(&pdev->dev, sizeof(int) * (fan_data->active_steps), GFP_KERNEL);
	if (!pwm_data) {
		err = -ENOMEM;
		goto pwm_alloc_fail;
	}

	of_err |= of_property_read_u32_array(node, "active_pwm", pwm_data, (size_t) fan_data->active_steps);
	...
	fan_data->workqueue = alloc_workqueue(dev_name(&pdev->dev), WQ_HIGHPRI | WQ_UNBOUND, 1);
	...
	fan_data->precision_multiplier = fan_data->pwm_period / fan_data->fan_pwm_max;
	dev_info(&pdev->dev, "cap state:%d, cap pwm:%d\n", fan_data->fan_state_cap, fan_data->fan_cap_pwm);

	fan_data->cdev = thermal_cooling_device_register("pwm-fan", fan_data, &pwm_fan_cooling_ops);
	...
	if (IS_ERR(fan_data->pwm_dev) && PTR_ERR(fan_data->pwm_dev) != -EPROBE_DEFER) {
	...
	if (of_err < 0)
		...
		fan_data->tach_workqueue = alloc_workqueue(fan_data->name, WQ_UNBOUND, 1);
		if (!fan_data->tach_workqueue) {
			err = -ENOMEM;
			goto tach_workqueue_alloc_fail;
		}
		INIT_DELAYED_WORK(&(fan_data->fan_tach_work), fan_tach_work_func);
	}
	...
	set_pwm_duty_cycle(fan_data->fan_pwm[fan_data->active_steps -1], fan_data);
	fan_data->fan_cur_pwm = fan_data->fan_pwm[fan_data->active_steps - 1];
